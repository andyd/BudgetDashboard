...
describe('formatCurrency', () => {
  ...
  describe('small amounts', () => {
    it('formats amounts under $1000 without abbreviation', () => {
      ...
      expect(formatCurrency(3.50)).toBe('$4'); // Rounds to integer
    });
  });
...
describe('generateComparisonSummary', () => {
  ...
  it('handles large numbers with proper formatting', () => {
    const comparison: Comparison = {
      ...
      unitCost: 8_500,
      result: 1_458_823,
      formula: '$12.4B ÷ $8.5K/year = 1,458,823 Individual Health Insurance Premiums',
      isFeatured: true,
      createdAt: new Date(),
      updatedAt: new Date(),
    };
...
describe('COMPARISON_UNITS', () => {
  ...
  it('all units have required fields', () => {
    COMPARISON_UNITS.forEach((unit) => {
      ...
      expect(['salary', 'healthcare', 'education', 'housing', 'general']).toContain(
        unit.category
      );
