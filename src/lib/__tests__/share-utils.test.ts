...
describe("share-utils", () => {
  ...
  describe("generateComparisonUrl", () => {
    it("should generate comparison URL with base URL", () => {
      const url = generateComparisonUrl("defense-2025:starbucks-latte");
      expect(url).toContain("/compare/defense-2025:starbucks-latte");
      expect(url).toMatch(/^https?:\/\//); // Should start with http:// or https://
    });

    it("should handle encoded comparison IDs", () => {
      const encoded = encodeComparison("item with spaces", "unit/with/slashes");
      const url = generateComparisonUrl(encoded);
  ...
  describe("generateBudgetUrl", () => {
    it("should generate budget drill-down URL", () => {
      ...
      expect(url).toContain("/budget/defense/military/navy");
      expect(url).toMatch(/^https?:\/\//); // Should start with http:// or https://
    });

    it("should handle single path segment", () => {
      const url = generateBudgetUrl(["defense"]);
      expect(url).toContain("/budget/defense");
  ...
  describe("getAllShareUrls", () => {
    it("should return all share URLs", () => {
      ...
      expect(result.url).toContain("/compare/defense-2025:starbucks-latte");
      expect(result.url).toMatch(/^https?:\/\//);
      expect(result.text).toContain(
        "The Defense Budget equals 160 billion lattes",
      );
