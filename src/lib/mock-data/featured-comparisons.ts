 ...
 *
 * All calculations are based on realistic 2025-2026 federal budget data.
 */
...
export const FEATURED_COMPARISONS: SimpleFeaturedComparison[] = [
  ...
  {
    // ICE facility expansion (FY2025 request): ~$420M for new facilities
...
export const FEATURED_COMPARISON_CONTEXT = {
  "ice-detention-operations": {
    source: "U.S. Immigration and Customs Enforcement FY2025 Budget",
    sourceUrl: "https://www.ice.gov/budget",
    methodology:
      "Based on average daily detention costs ($210/day) and average daily population (35,000)",
    lastUpdated: "2025-03-15",
  },
  "f35-procurement": {
    source: "Department of Defense FY2025 Budget Request",
    sourceUrl: "https://www.defense.gov/News/Releases/Release/Article/3284154/",
    methodology:
      "Annual procurement costs for F-35 Joint Strike Fighter program",
  ...
  "aircraft-carrier-ford-class": {
    ...
    methodology:
      "Total procurement cost for one Gerald R. Ford-class aircraft carrier",
    lastUpdated: "2024-12-08",
  },
  "ice-facility-expansion": {
    source: "DHS FY2025 Congressional Budget Justification",
    sourceUrl: "https://www.dhs.gov/budget",
    methodology:
      "Requested funding for new ICE detention facility construction and expansion",
    lastUpdated: "2025-03-15",
  },
  "defense-rnd": {
    ...
    methodology:
      "Total Research, Development, Test & Evaluation appropriations",
    lastUpdated: "2025-03-28",
  },
};
...
export function getFeaturedComparisons(): SimpleFeaturedComparison[] {
  return [...FEATURED_COMPARISONS].sort(
 ...
 */
export function getFeaturedComparisonsByBudgetItem(
  budgetItemId: string,
): SimpleFeaturedComparison[] {
  return FEATURED_COMPARISONS.filter(
    (comp) => comp.budgetItemId === budgetItemId,
  );
}
